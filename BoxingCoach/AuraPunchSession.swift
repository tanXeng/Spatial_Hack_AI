...
enum AuraPunchPhase: String, Sendable {
    ...
    case acquiring
    /// The coach character demonstrates the punch, then keeps throwing it beside the user.
    case coachDemo
...
final class AuraPunchSession {
    ...
    /// Introduces the coach: one clean full-speed rep, then the same punch on a loop that runs
    /// for the whole guided follow-along.
    ///
    /// The ghost's hold-until-matched loop is still what teaches the motion; the coach is the
    /// full-body "here is what it looks like" alongside it, which is why the clips do not need
    /// segmenting into out/hold/return the way the ghost's trajectory does.
    ///
    ...
    private func runCoachDemo(solver: ArmPoseSolver) async {
        ...
        // He *stays on screen* from here, still punching, for the whole guided follow-along;
        // `dismissCoach()` takes him away only once the user is throwing unaided.
