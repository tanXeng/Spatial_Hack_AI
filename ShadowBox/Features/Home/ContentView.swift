...
struct ContentView: View {
    ...
    private var entryAllowed: Bool {
        ...
        if !DeveloperConfig.isOfflineModeEnabled,
           !safetyAcknowledged || !profileStore.hasProfile {
            return false
        }
        if currentExperience == .bagPreview {
            guard !DeveloperConfig.isOfflineModeEnabled else {
    ...
    private var entryStatusText: String {
        ...
        if DeveloperConfig.isOfflineModeEnabled {
            return safetyAcknowledged ? "Offline mode: deterministic features are enabled" : "Complete the safety acknowledgement"
        }
        if !profileStore.hasProfile {
    ...
    private func choose(_ feature: TrainingFeature) {
        if feature != .anthropometry,
           !DeveloperConfig.isOfflineModeEnabled,
    ...
    private func featureBadge(_ feature: TrainingFeature) -> String {
        ...
        case .auraPunch:
            profileStore.hasProfile ? "Jab + Cross" : "Offline-ready"
        case .reactiveStrike:
            "3 activities"
        }
    }
