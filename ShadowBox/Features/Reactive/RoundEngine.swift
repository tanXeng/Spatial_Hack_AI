...
final class RoundEngine {
    ...
    func setTrackingState(_ state: HandTrackingState) {
        if DeveloperConfig.isOfflineModeEnabled {
            ...
            ensureOfflineCalibration()
            if phase == .setup {
                phase = .ready
                instruction = "Offline mode: hand tracking and calibration are running in local deterministic mode."
            }
            return
        }
    ...
    private func ensureOfflineCalibration() {
        guard calibration == nil else { return }

        let reach = max(
            configuration.minimumComfortableReach,
            min(
                DeveloperConfig.offlineCalibrationComfortableReach,
        ...
        let profile = CalibrationProfile(
            ...
            leftGuard: SIMD3<Float>(
                -DeveloperConfig.offlineGuardLateralOffset,
                DeveloperConfig.offlineGuardHeight,
                -0.82
            ),
            ...
            comfortableReach: reach,
            straightPunchDirection: SIMD3<Float>(0, 0, -1),
            referenceStraightSpeed: max(
                configuration.minimumReferenceSpeed,
                DeveloperConfig.offlineReferenceSpeed
            )
        )
        ...
        functionalCalibrationReports = [
            .left: FunctionalCalibrationReport.evaluate(
                reachMeters: [reach, reach],
                thresholds: functionalCalibrationThresholds
            ),
            .right: FunctionalCalibrationReport.evaluate(
                reachMeters: [reach, reach],
                thresholds: functionalCalibrationThresholds
            ),
        ]
        reachRepetitions = [
            .left: [
                ReachRepetition(
                    ...
                    measurement: ReachMeasurement(
                        delta: profile.straightPunchDirection(for: .left) * reach,
                        distance: reach
                    ),
                    peakSpeed: max(
                        configuration.minimumReferenceSpeed,
            ...
            .right: [
                ReachRepetition(
                    hand: .right,
                    measurement: ReachMeasurement(
                        delta: profile.straightPunchDirection(for: .right) * reach,
                        distance: reach
                    ),
                    peakSpeed: max(
                        configuration.minimumReferenceSpeed,
        ...
        instruction = "Offline mode: deterministic calibration is active. Guard and reach calibration are prefilled."
        trackingAvailable = true
    }
